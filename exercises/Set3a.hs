...
interpreter commands = go commands 0 0
  where go [] x y = []
        go (cmd:cmds) x y
          | cmd == "up" = go cmds x (y+1)
          | cmd == "down" = go cmds x (y-1)
          | cmd == "left" = go cmds (x-1) y
          | cmd == "right" = go cmds (x+1) y
          | cmd == "printX" = show x : go cmds x y
          | cmd == "printY" = show y : go cmds x y
          | otherwise = go cmds x y
