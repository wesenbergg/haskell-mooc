...
longest :: Ord a => [[a]] -> [a]
longest xs = maximumBy (\x y -> compare (length x, Down (head x)) (length y, Down (head y))) xs
...
transfer from to amount bank =
  ...
  in if amount < 0 || not (Map.member from bank) || not (Map.member to bank) || fromBalance < amount
     then bank
     else Map.insert to (Map.findWithDefault 0 to bank + amount) (Map.insert from (fromBalance - amount) bank)
