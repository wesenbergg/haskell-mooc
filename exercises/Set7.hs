...
data State = Start | Error | Finished | HasEggs | HasEggsAndFlour | HasEggsAndSugar | HasAllIngredients | Mixed
  deriving (Eq,Show)
...
step HasAllIngredients Mix = Mixed
...
evaluate (Operation "*" left right) = evaluate left * evaluate right
evaluate (Operation _ _ _) = error "Unknown operation"
